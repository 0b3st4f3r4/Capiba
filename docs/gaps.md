...
- [x] **Conectar operadores de grafo ao pipeline** — `detect_collusion` e `trace_ownership` reescritos na semântica adaptada e **validados empiricamente** (bateria D-02, `docs/results/R-D-02.md`, 6/6) e agora conectados: o `task_detect` emite o sinal `collusion_network` por par de fornecedores (`collusion_signals` em `detection/signals.py`, score binário 1.0 como placeholder de calibração, limiar `DETECTION_COLLUSION_MIN_WINS=3`, best-effort — ArangoDB fora não derruba a task) e a API expõe `GET /v1/graph/ownership/{cnpj}?max_depth=3` (router `api/routers/graph.py`, 503 com ArangoDB indisponível). `anomalous_geography` segue fora, sem fonte de lat/long
