...
Chunk: tse
Responsibility: Download the prestação de contas eleitorais ZIP of the
configured election year from the TSE open data CDN.
...
from capiba.config import TSE_BASE_URL, TSE_ELECTION_YEAR
...
def download_tse_dump(
    ...
    """Downloads the TSE prestação de contas dump ZIP of an election year.
    ...
    Args:
        ...
        base_url: Base URL of the prestação de contas directory on the TSE
            CDN.
        files: File names to download. Default: the candidates' dump of
            ``year`` (``tse_dump_filename``).
    ...
    files = files if files is not None else [tse_dump_filename(year)]
    skip = skip or set()
